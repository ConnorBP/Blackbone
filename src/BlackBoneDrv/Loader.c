...
NTSTATUS BBLookupProcessThread( IN PEPROCESS pProcess, OUT PETHREAD* ppThread )
{
    ...
    PVOID pBuf = ExAllocatePool2( NonPagedPool, 1024 * 1024, BB_POOL_TAG );
    ...
    )
{
    ...
    PKAPC pInjectApc = ExAllocatePool2( NonPagedPool, sizeof( KAPC ), BB_POOL_TAG );
    ...
    if (bForce)
    {
        pPrepareApc = ExAllocatePool2( NonPagedPool, sizeof( KAPC ), BB_POOL_TAG );
        KeInitializeApc(
...
NTSTATUS BBMapWorker( IN PVOID pArg )
{
    ...
    if (NT_SUCCESS( status ))
        fileData = ExAllocatePool2( PagedPool, fileInfo.EndOfFile.QuadPart, BB_POOL_TAG );
    else
