...
NTSTATUS BBAllocateFreePhysical( IN PEPROCESS pProcess, IN PALLOCATE_FREE_MEMORY pAllocFree, OUT PALLOCATE_FREE_MEMORY_RESULT pResult )
{
    ...
    if (pAllocFree->allocate != FALSE)
    {
        ...
        pRegionBase = ExAllocatePool2( NonPagedPool, pAllocFree->size, BB_POOL_TAG );
        if (!pRegionBase)
        ...
        if (pResult->address)
        {
            ...
            if (pEntry == NULL)
            {
                pEntry = ExAllocatePool2( PagedPool, sizeof( MEM_PHYS_PROCESS_ENTRY ), BB_POOL_TAG );
            ...
            pMemEntry = ExAllocatePool2( PagedPool, sizeof( MEM_PHYS_ENTRY ), BB_POOL_TAG );
